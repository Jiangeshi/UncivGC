...
object UgcPediaContent {
    ...
    private val ugcChanges = listOf(
        ...
        CivilopediaCategories.UgcPediaEntry("商路系统", listOf(
            point("城市与城市直接建立商路连接（无需商路单位），单向计费，双向可各连一次"),
            point("发起方（卖货）：金币 = 已改良地块资源数 + 距离×系数（陆商 1.5/格、海商 0.5/格）"),
            point("接收方（进货）：文化 +1×奢侈数、产能 +1×战略数、食物 +0.5×奖金数（发起方已改良资源）"),
            point("容量：时代序号 +「提供 X 条贸易路线」词条（市场/港口各提供 1 条）"),
            point("收益加成全部生效：词条「每条商路 +X」固定收益、「贸易路线 +X%」百分比加成，商路页显示总收益"),
            point("同盟 Lv1 起：同盟间商路收益 +50%"),
            point("连接城邦：该城邦对你的影响力 +5/条（永久不掉）"),
            point("玩家之间外商需申请（接受/拒绝）；内商/城邦/AI 自动建立；断开冷却 3 回合"),
            point("发起条件：本城需有已开发的地块资源"),
        ), 14),
        CivilopediaCategories.UgcPediaEntry("同盟系统", listOf(
            point("双文明结盟（成对 1 对 1，可同时与多个文明分别结盟），期限 20 回合"),
            point("到期双方弹窗续约：都同意 → 等级+1、期限重置；本回合不响应视为拒绝"),
            point("Lv1：盟友已研究科技 +10% 科研、同盟间商路收益 +50%"),
            point("Lv2：与盟友共同对敌时 +10% 战斗力"),
            point("Lv3：共享实时视野（不含探索历史）"),
            point("同盟期间不能宣战/攻击/劫掠盟友，无法主动退出"),
            point("拒绝提议 / 不续约后，与该文明同盟冷却 10 回合"),
            point("盟友被宣战时弹窗询问是否跟进参战"),
        ), 15),
    ...
    private val ugcCredits = listOf(
        CivilopediaCategories.UgcPediaEntry("致谢", listOf(
            ...
            gap(),
            line("特别感谢：", color = "#9cf"),
            line("感谢浮泽云梦熙、冷雨之风、维德、满堂花醉、溯泯、Excuse me？等提供的额外支持！"),
        ), 1),
    )
}
