...
object UgcPediaContent {
    ...
    private val ugcChanges = listOf(
        ...
        CivilopediaCategories.UgcPediaEntry("商路系统", listOf(
            ...
            point("发起方（卖货）：金币 =（已改良资源数×0.5 + 距离×系数 + 词条加成）×（1+接收方人口/20）× 同盟 × 1/rank"),
            point("接收方（进货）：文化 +1×奢侈数、产能 +1×战略数、食物 +0.5×奖金数（发起方已改良资源）×（1+发起方人口/20）× 同盟 × 1/rank"),
            point("每条商路按发起方先后顺序 1/rank 衰减：第 1 条全额、第 2 条 ×1/2、第 3 条 ×1/3…（双向各自独立）"),
            point("容量：时代序号 +「提供 X 条贸易路线」词条（市场/港口各提供 1 条）"),
            point("词条加成全部生效：固定收益（每条商路 +X）并入基础吃全系数（只发起方有）、百分比加成（贸易路线 +X%）乘在总收益上"),
            point("同盟 Lv1 起：同盟间商路收益 +25%"),
            point("连接城邦：该城邦对你的影响力 +10/条（不吃衰减，多条叠加）"),
