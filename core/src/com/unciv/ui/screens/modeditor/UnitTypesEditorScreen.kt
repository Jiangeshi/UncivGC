...
/** 单位类型编辑器：name / movementType (Land/Water/Air) / uniques / civilopediaText */
class UnitTypesEditorScreen(private val modFolder: FileHandle) : BaseScreen() {

    private val items = ModEditorData.loadUnitTypes(modFolder)
    ...
    private lateinit var movementTypeBox: ModEditorSelectBox
    ...
    private fun getDisplayGroup(item: ModObjectData): DisplayGroup = when (item.getString("movementType")) {
    ...
    init {
        ...
        topBar.add(("UnitTypes".tr() + " · UnitTypes.json").toLabel(fontSize = 28))
        ...
        val addButton = "+ New unit type".toTextButton()
        ...
        else formTable.add("No unit types. Click \"+ New unit type\" in the top left.".toLabel()).pad(20f).row()
    }
    ...
    private fun refreshList() {
        ...
        for (group in DisplayGroup.entries) {
            ...
            val colorBar = Table(BaseScreen.skin).apply {
                background = BaseScreen.skinStrings.getUiBackground(
                    "ModEditor/UTGroupColor_${group.name}", null, group.color)
            }
        ...
        if (items.isEmpty()) {
            listTable.add("No unit types yet.".toLabel()).pad(20f).row()
        }
    }
    ...
    private fun rebuildForm() {
        ...
        val header = Table(BaseScreen.skin)
        header.add("Edit unit type".toLabel(fontSize = 24)).left().expandX()
        ...
        movementTypeBox = ModEditorSelectBox(listOf("Land", "Water", "Air"), item.getString("movementType") ?: "Land", searchable = true)
        movementTypeBox.setAlignment(com.badlogic.gdx.utils.Align.left)
        movementTypeBox.selected = movementTypeBox.items.firstOrNull { it.value == item.getString("movementType") }
            ?: movementTypeBox.items.first()
        movementTypeBox.addListener(object : ChangeListener() {
            override fun changed(event: ChangeEvent?, actor: Actor?) {
                item.raw["movementType"] = movementTypeBox.selected?.value ?: "Land"
                expandedGroups.add(getDisplayGroup(item))   // 先加入新组再刷新，否则新组不展开（2026-08-19）
                refreshList()
            }
        })
        val mtRow = Table(BaseScreen.skin)
        mtRow.add("movementType".toLabel()).left().pad(4f).width(220f)
        mtRow.add(movementTypeBox).growX().minWidth(200f).pad(4f)   // 行内控件必须 growX，否则右侧空白
        formTable.add(mtRow).fillX().left().pad(4f, 10f, 4f, 10f).row()
    ...
    private fun save() {
        ...
        val problems = ModEditorData.validateUnitType(modFolder, item, items)
    ...
    private fun doSave() {
        ModEditorData.saveUnitTypes(modFolder, items)
        val gameProblems = ModEditorData.runGameValidation(modFolder)
        val filtered = ModEditorData.filterGameProblems(gameProblems, "UnitTypes.json")
        val errors = filtered.filter { it.second }
        if (errors.isNotEmpty()) {
            ModEditorData.rollbackFile(modFolder, "UnitTypes.json")
    ...
    private fun showUniqueEditor(index: Int, currentRaw: String) {
        val popup = com.unciv.ui.popups.Popup(this)
    ...
    private fun addItem() {
        ...
        item.name = "New unit type"
        item.raw["name"] = item.name
        item.raw["movementType"] = "Land"
        items.add(item)
        selectedIndex = items.lastIndex
    ...
    private fun showCopyPopup(initialSource: String? = null) {
        ...
        val base = ModEditorData.loadBaseObjects(modFolder, "UnitTypes.json", sourceRuleset)
        if (base.isEmpty()) {
            showInfoPopup("No unit types found in the base ruleset")
            return
        }
        val popup = com.unciv.ui.popups.Popup(this)
        ...
        sourceBox.addListener(object : com.badlogic.gdx.scenes.scene2d.utils.ChangeListener() {
            override fun changed(event: com.badlogic.gdx.scenes.scene2d.utils.ChangeListener.ChangeEvent?, actor: com.badlogic.gdx.scenes.scene2d.Actor?) {
                popup.close()
                showCopyPopup(initialSource = sourceBox.selected.value)
            }
        })
        ...
        popup.add("Copy unit type from ruleset".toLabel(fontSize = 22)).colspan(2).row()
