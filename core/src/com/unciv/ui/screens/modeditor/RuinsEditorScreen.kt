...
/** Ruins 编辑器：古代遗迹奖励（列表 + 表单，平铺风格，2026-08-19） */
class RuinsEditorScreen(private val modFolder: FileHandle) : BaseScreen() {

    private val items = ModEditorData.loadRuins(modFolder)
    ...
    private lateinit var notificationArea: TextArea
    private lateinit var weightField: UncivTextField
    private lateinit var colorField: UncivTextField
    ...
    init {
        ...
        topBar.add(("Ruins".tr() + " · Ruins.json").toLabel(fontSize = 28))
        ...
        val addButton = "+ New ruin".toTextButton()
        addButton.onActivation { addItem() }
        buttonRow.add(addButton).left().pad(6f)
        val copyButton = "Copy ruin from ruleset".toTextButton()
        ...
        else formTable.add("No ruins. Click \"+ New ruin\" in the top left.".toLabel()).pad(20f).row()
    }
    ...
    private fun rowBackground() = BaseScreen.skinStrings.getUiBackground(
        "ModEditor/RuinRow", BaseScreen.skinStrings.roundedEdgeRectangleSmallShape,
    ...
    private fun refreshList() {
        ...
        if (items.isEmpty()) {
            listTable.add("No ruins yet.".toLabel(fontSize = 16, fontColor = Color(1f, 1f, 1f, 0.4f))).pad(20f).row()
        }
    }
    ...
    private fun nextName(): String {
        ...
        var i = 1
        while (("New ruin" + if (i == 1) "" else " $i") in existing) i++
        return "New ruin" + if (i == 1) "" else " $i"
    }
    ...
    private fun rebuildForm() {
        ...
        val header = Table(BaseScreen.skin)
        header.add("Edit ruin".toLabel(fontSize = 24)).left().expandX()
        ...
        nameField = UncivTextField("Ruin name (required)", item.name)
        ...
        val nameHint = "Never shown to the user, but names must be distinct".tr()
            .toLabel(fontSize = 12, fontColor = Color(1f, 1f, 1f, 0.45f))
        nameHint.wrap = true
        formTable.add(nameHint).growX().left().pad(0f, 188f, 6f, 8f).row()

        // notification（必填，可用 [参数] 占位符）
        notificationArea = TextArea(item.getString("notification"), BaseScreen.skin)
        val notifScroll = AutoScrollPane(notificationArea).apply {
            setOverscroll(false, false)
            setScrollingDisabled(true, false)
        }
        val notifRow = Table(BaseScreen.skin)
        notifRow.add("Notification".tr().toLabel()).left().pad(4f).width(180f).top()
        notifRow.add(notifScroll).growX().height(80f).pad(4f)
        formTable.add(notifRow).growX().left().row()

        weightField = numberField(item.getIntText("weight"))
        colorField = UncivTextField("", item.getString("color"))
        val row2 = Table(BaseScreen.skin)
        addStatPair(row2, "Weight", weightField)
        val colorRow = Table(BaseScreen.skin)
        colorRow.add("Color".tr().toLabel()).left().pad(4f).width(180f)
        colorRow.add(colorField).growX().minWidth(160f).pad(4f)
        formTable.add(colorRow).growX().left().row()
        val weightHint = "Weight: relative chance this reward is chosen; 0 means never".tr()
            .toLabel(fontSize = 12, fontColor = Color(1f, 1f, 1f, 0.45f))
        weightHint.wrap = true
        formTable.add(weightHint).growX().left().pad(0f, 8f, 6f, 8f).row()

        // excludedDifficulties（chips 多选）
        ModEditorListSection(
            screen = this,
            label = "excludedDifficulties",
            options = { ModEditorData.getDifficulties(modFolder) },
            getValues = { item.getStringList("excludedDifficulties") },
            setValues = { item.setStringList("excludedDifficulties", it) }
        ).addTo(formTable)
        ...
        rebuildUniquesTable()
        val uniqueHint = "If more than 1 unique is added, the notification will be shown multiple times due to a game bug".tr()
            .toLabel(fontSize = 12, fontColor = Color(1f, 1f, 1f, 0.45f))
    ...
    private fun save() {
        ...
        if (newName.isBlank()) { showMessage("Ruin name cannot be empty".tr()); return }
        item.name = newName
        item.setString("name", newName)
        item.setString("notification", notificationArea.text.trim())
        val weight = weightField.text.trim()
        if (weight.isBlank()) item.raw.remove("weight")
        else {
            val v = weight.toIntOrNull()
            if (v == null) { showMessage("Invalid number:".tr() + " " + weight); return }
            item.setInt("weight", v)
        }
        val color = colorField.text.trim()
        if (color.isBlank()) item.raw.remove("color") else item.setString("color", color)
        item.comment = commentArea.text
        item.syncUniques()

        val problems = ModEditorData.validateRuin(modFolder, item, items)
    ...
    private fun doSave() {
        ModEditorData.saveRuins(modFolder, items)
        val gameProblems = ModEditorData.filterGameProblems(
            ModEditorData.runGameValidation(modFolder), "Ruins.json")
        val errors = gameProblems.filter { it.second }
        if (errors.isNotEmpty()) {
            ModEditorData.rollbackFile(modFolder, "Ruins.json")
    ...
    private fun showCopyFromRulesetPopup(initialSource: String? = null) {
        ...
        val baseItems = ModEditorData.loadBaseObjects(modFolder, "Ruins.json", sourceRuleset)
        if (baseItems.isEmpty()) { showMessage("No ruins found in the base ruleset"); return }
        val popup = Popup(this, scrollable = Popup.Scrollability.None)
        ...
        popup.add("Copy ruin from ruleset".tr().toLabel(fontSize = 20)).pad(8f).row()
    ...
    private fun confirmDelete() {
        ...
        popup.addButton("Delete".tr()) {
            ...
            if (items.isEmpty()) {
                ...
                refreshList()
                formTable.clear()
                formTable.add("No ruins. Click \"+ New ruin\" in the top left.".toLabel()).pad(20f).row()
            } else {
